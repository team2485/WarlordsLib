  ...
  public double getOutput(double input) {
    ...
    if (input < points.get(0).X) { // here, we extrapolate from the lowest two points
      bottomI = 0;
        ...
        >= points.get(points.size() - 1).X) { // here, we extrapolate from the top two points
      bottomI = points.size() - 2;
    } else {
      for (int i = 0;
          i < points.size() - 1;
          i++) { // here, we interpolate from the points bordering the given input
    ...
    return points.get(bottomI).Y
        + (input - points.get(bottomI).X)
            * (points.get(bottomI + 1).Y - points.get(bottomI).Y)
            / (points.get(bottomI + 1).X - points.get(bottomI).X);
  }
}
