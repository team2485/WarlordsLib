...
public class BufferZone {
   ...
   * @param bufferSize the position range at which the buffer will begin linearly limiting the
   *     output
   */
  public BufferZone(
      double minOutput,
  ...
  public double get(double input, double position) {
    return MathUtil.clamp(
        input,
        Math.min(
            Math.max(m_minOutput, (m_minOutput / m_bufferSize) * (position - m_minPosition)), 0),
        Math.max(
            Math.min(m_maxOutput, -(m_maxOutput / m_bufferSize) * (position - m_maxPosition)), 0));
  }
}
